...
$execute as @e[type=marker,tag=mwe.forcefield,limit=1,sort=nearest] at @s positioned ~ ~0.5 ~ facing entity @e[type=marker,tag=mwe.ff.impact,limit=1] feet run function moreworldevents:items/forcefield/dense_rings {radius:$(radius)}
