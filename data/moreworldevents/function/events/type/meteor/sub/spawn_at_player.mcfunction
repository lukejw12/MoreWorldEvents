...
execute if score #mwe.event_type/sub mwe.event_type matches 2 run function moreworldevents:events/type/meteor/sub/summon_meteors {count:20}
...
execute as @e[type=fireball,tag=mwe.meteor] at @s run spreadplayers ~ ~ 5 20 false @s
execute as @e[type=fireball,tag=mwe.meteor] at @s run tp @s ~ ~80 ~
